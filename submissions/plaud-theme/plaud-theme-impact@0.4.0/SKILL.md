...
description: PLAUD Shopify 主题矩阵的 Assess 阶段（order 2）：改动影响面侦察。 用户问"改这个会影响什么""影响范围多大""blast radius""波及哪些模板/页面""依赖树""上下游调用方" "这个 snippet 谁在用""改共享 snippet / 全局 CSS / token 会不会炸全站""旧 section 有没有连带影响" "这次改动能不能到店""改了各店收得到吗""17 个店都会生效吗""保护清单""受保护路径""同步会不会覆盖" "改 templates/*.json 各店能收到吗""改 locales 文案店里会变吗""PageFly / pf-* / matycube 文件能不能改" "站点自研代码""某个店自己写的文件会不会冲突""sync 不覆盖" "有多少实例""实例数""disabled 实例算不算""改动风险高不高""该动模板存值还是 schema 还是模块代码" 时使用；Path A 改已有 section/snippet/CSS、Path B 新建 sa-* section 查复用面与冲突面、 Path C UX 迁移前的 Blast Radius 与实例审计，都先过本 skill。 产出 LegacyImpact / IntegrationSurface / InlineLite 三种 ReconMode 下的事实： 理论引用数 vs 实际受影响实例清单、启用/disabled 实例分列、依赖树、共享传播链、 token→class 全实例覆盖核查、dangling 引用核查、build 产物 vs 源文件区分、 可选修改入口与各自风险、RiskTier、应跑的 QA profile， 以及 **v0.4.0 新增的 SyncReach**：本次改动能不能通过「基线 origin/main → 17 个独立 Shopify 店」的 sync 到店 —— 逐改动项比对 15 条保护规则与两条例外，区分 templates 的文件级（只有基线新增的模板能过）与 locales 的字段级（新增/删除字段能过、改已本地化的键值过不去），并指出站点自研代码造成的影响面下界偏差。 SyncReach 只判「这条路径能不能过 sync」，**不推断要推哪几个站**（那仍由 plaud-theme-qa-intake 按运营出处填）， 也不出逐店落地方案（那是提测材料 SyncReachStatus）。 本 skill 只产出事实与证据命令：不改任何代码、不写 section、不做 UX 迁移、 不下 RootCause 根因结论、不选实现方案、不做验收、不判定可交付、不批准模板存值编辑。 不用于非 Plaud 主题、Hydrogen/headless、Shopify App/Admin/Checkout Extension、WooCommerce。
---
...
本次计划写入集是否包含以下任一？
  ...
  └─ 否
      ...
      └─ 单文件 + 9 条豁免条件全满足（含 sync-reach 保护规则未命中） → InlineLite（需逐条自证）
```
...
- [ ] 不改 schema
- [ ] 不改模板存值
- [ ] 🔴 **该文件不命中 `plaud-theme-shared/references/sync-reach.md` §2 的 15 条保护规则**（落在 §2.1 例外内的不算命中）

> **拿不准就不是 InlineLite。** InlineLite 是"没什么可评估"的声明，不是"我懒得评估"的出口。豁免成立时，实现 skill 内联完成评估并在自己的 HandoffContract 写明 `ReconMode: InlineLite` + 豁免理由；本 skill 若被调用，仍要把上面 9 条的核查结果和 `EvidenceCommands` 交出去。

> 🔴 **最后一条（v0.4.0 新增）不是凑数的。** 前 8 条判据全是「这个文件在**仓库内**没有别的引用方」，
> 而到不到店是**仓库外**的分发事实——仓库里怎么查都查不出来。
> 一个只改 `snippets/ga4-push.liquid` 的单文件改动完全满足前 8 条，却是 17 个店一个都收不到的典型形态。
...
## 第四步半 — SyncReach：这次改动到不到店（v0.4.0 新增）

> 🔴 **规则、匹配语法、例外与判定算法的唯一事实源是 `plaud-theme-shared/references/sync-reach.md`。**
> 本节只讲怎么在本 skill 的流程里执行它，**不复制规则表**（复制 = 第二个事实源，规则一改必然漂移）。

PLAUD 是「一套基线 `origin/main` → sync 到 **17 个独立 Shopify 店**」。**15 条全局保护规则**圈住的文件
（`templates/**/*.json`、`locales/*.json`、`sections/*.json`、`config/*.json` 与一批埋点 snippet 等）
sync **不覆盖各店版本**。改了它们，基线变了、各店没变，而 Theme Check / QA / 推站**全部正常绿灯**。

前面四步回答的是「改这个会影响基线里的什么」。本步回答一个正交的问题：**「这些改动到底出不出得了基线」**。

### 做什么

对本次**计划写入集**里的每一条路径（不是 `git diff` —— Assess 发生在实现之前）：

1. 归一化路径 → 逐条比对 15 条规则 → 记 `MatchedRules[]`（多重命中全列，不造组合值）
2. 比对 §2.1 的两条例外（`config/settings_schema.json`、`locales/*.schema.json` 仍走共享同步）
3. 例外优先，定 `EffectiveRule` → 查出 `Classification`
4. `templates/**/*.json` 再判 `ChangeKind`（`Added` 才可能过得去，判据要 `BaseHeadSha`）
5. `locales/*.json` **必须逐 JSON key 展开**（`sync-reach.md` §5.3）—— 路径级一个结论表达不了「哪些 key 到得了店」
6. 🔴 **比对站点自研代码清单**（`sync-reach.md` §5.1 第 6/7 步）：路径与某个店的自研文件**同名** → 记 `OverriddenAtSites`；
   **glob 侧结论是 `Reachable` 时**才改判 `SiteOverridden`（glob 已判不可达的保留原结论，但 `OverriddenAtSites` 照记）。
   - 🔴 **必须在第 3 步算完 `Classification` 之后做**：放在前面，第 3 步会把结论重新算成 `Reachable`，
     正好覆盖掉这一步要拦的那类路径（`sync-reach.md` §5.1 那段「早期草稿」讲的就是这个错法）。
   - 🔴 **这一步与 glob 无关** —— 15 条 glob 只覆盖「保护清单」，覆盖不到「站点自研代码」那一类。
     漏了它，`sections/us-form-contact-sales-2.liquid` / `assets/affirmShopify.js` 会被判成 `Reachable`，
     而 LATAM 与 US 各有一个同名自研文件、永远拿不到基线版。
   - 🔴 **`OverriddenAtSites` 非空就要如实记，不管最终 `Classification` 是什么**（`sync-reach.md` §4.2 末行铁律）：
     下游据它对 `OverriddenAtSites ∩ TargetSites` 里的店要落地方案。
     只在 `SiteOverridden` 时才填，会漏掉「`LocaleFieldLevel` + `Added` 到得了店、但碰撞店收不到」这一类。
   - 拿不到清单、或它的 `AsOf` 过旧 → `Undetermined`，**不得**默认「没查到同名 = 没有碰撞」
7. 命中匹配语法的解释约定边界 → `Undetermined`（覆盖上面所有结论）

### 三个最容易判错的地方

| 形态 | 正确结论 |
|---|---|
| 改 `templates/xxx.json` 里已有模板的存值 | **过不去**。#15 只同步基线**新增**的模板，各店已本地化的不覆盖 |
| 改 `locales/en.default.json` 里一句已有文案 | **过不去**。#5 的粒度是**字段级**：新增/删除 key 能过，已存在 key 的 value 不被覆盖。在 git diff 里它只是一行，看起来无害 |
| 改 `config/settings_schema.json` | **能过**。它被 #4 圈住但落在 §2.1 例外内，仍走共享同步 |

### 🔴 覆盖等式（最容易被绕过去的一条）

`SyncReach` 里出现的**可发布路径集合**必须**恰好等于本次计划写入集**的可发布路径集合
（`sync-reach.md` §5.3.1）。少一条 = 那条路径根本没判过；多一条 = 拿的是上一轮的结果。两个方向都要停。

> 🔴 **为什么它比「零命中要留证据」更重要**：交一份**只列了可达路径**的 `SyncReach`，
> 下游看到的是「所有项都 `Reachable`」→ `SyncReachStatus: NotApplicable` → `SyncReachCheck: Passed`，
> 三道门全绿，而真正会出事的那几条路径**从来没有出现在任何一份工件里**。
> 绕过这一条不需要说任何一句假话，只需要少写几行——这正是它必须写成机械等式的理由。

`locales/*.json` 还要多两条：展开的 key 集合必须覆盖该文件本次**全部**变更的 key；重命名必须是
`Deleted`(旧) + `Added`(新) **两条**。

### 两条硬约束

- 🔴 **零命中也要逐路径列出并标 `Reachable` + `MatchedRules: []`**，`Evidence` 写明比对过全部 15 条。
  一句「本次不涉及受保护路径」与「没查过」在文本上不可区分。
- 🔴 **任一条 `Undetermined` → `ReadyForImplement: No` + `BlockingGaps` 非空。**
  「先按能到店做，回头再确认」是这一步最想拦掉的失败模式。

### 本步**不**做的事

- **不推断站点清单。** `TargetSites` / `ExcludedSites` / `ThemeIds` 仍由 `plaud-theme-qa-intake` 按运营出处填。
  「这个文件能不能过 sync」是规则匹配的事实，「要推哪几个店」是有出处的决策——两者正交，别互相代入。
- **不出落地方案。** 「这条不可达的项各店怎么落地」是提测材料（`SyncReachStatus`，§9.1.2），不是 Assess 的事实。

### 站点自研代码：本 skill 的影响面有系统性下界偏差

GLOBAL / JP / LATAM / TW / US 各有**基线里根本不存在**的自研 `.liquid`/`.js`/`.css`（清单在项目侧
`memory/站点自研代码清单.md`，种子见 `plaud-theme-shared/references/memory-seed/`）。
它们不在依赖树里，因为依赖树只扫基线仓库。

- 改共享 snippet / 全局 CSS / token，或**删除、重命名** snippet → 那几个店的实际影响面 **≥** 本 skill 算出的值。
- 基线**新建** `sections/` `assets/` 下的文件 → 先核有没有同名自研文件。已知碰撞：
  `sections/us-form-contact-sales-2.liquid` 与 `assets/affirmShopify.js` 同时存在于 **LATAM 和 US**——
  引擎结构性避让，那两个店永远拿不到基线版，而基线侧一切检查正常。

把这一条写进 `SharedPropagation` 的说明文字，**不得**默认「基线里没引用 = 没人用」。
拿不到那份清单、或它的 `AsOf` 太旧 → `Undetermined`，停机要。
...
> 🔴 **v0.4.0：排序改成两级。** 先按第四步半的 `SyncReach` **筛掉不可达入口**，再在可达入口里按下表排。
> 多店基线下第一档「模板存值」恰好是同步过不去的那一档（`handoff-schema.md` §8.1 #8 已整条改写）。
> 三层**没有任何一个**可达时 → 停机，交运营决定走逐店手工落地还是改需求（第七步）。

| 层 | 影响范围 | 风险 | 前置 | 到店 |
|---|---|---|---|---|
| 模板存值（`templates/*.json` 实例值） | 仅该模板该实例 | 低 | `templates/*.json` 默认只读，实际编辑需用户明确批准 | 🔴 **改已有模板到不了店**（`NewTemplateOnly`） |
| schema 配置（section liquid 内 schema） | 该模块所有**新建**实例；旧 stored 值不变但需向后兼容 | 中 | — | 可达（`sections/*.liquid` 不在保护清单里） |
| 模块代码（liquid / SCSS / JS） | 该模块**所有命中该路径的现存实例** | 高 | 可能触发 build | 可达 |

每个候选只报**事实**：要改哪类文件、波及哪些既有/新建实例（引用已核实的清单）、是否需要模板授权、是否产生向后兼容要求、是否触及 build 源、**该入口的 `SyncReach` 结论**、当前还缺哪些证据。
...
- 🔴 **v0.4.0**：`SyncReach` 的路径集合与本次计划写入集**不相等**（少判了路径，或混进了上一轮的路径）→ 停，补齐后重出（`sync-reach.md` §5.3.1）
- 🔴 **v0.4.0**：`SyncReach` 里出现任一 `Undetermined` → 停，写明缺什么。四种来源：① 命中 `sync-reach.md` §3 里标 🟠 的解释约定边界（`matycube*`/`pagefly*` 是前缀还是 substring、`**/` 是否含零层、大小写）；② `BaseHeadSha` 不可解析，判不了 `templates/**/*.json` 的「新增 vs 修改」；③ `memory/站点自研代码清单.md` 缺失或 `AsOf` 太旧；④ 报告未覆盖的路径形态（`blocks/`、多层 `locales/`、symlink 等）
- 🔴 **v0.4.0**：三层入口里**没有任何一个**能到达目标店 → 停，交运营决定走逐店手工落地还是改需求。**不得**自行选一个不可达入口列成候选然后放行
...
| **判 `SyncReach`：15 条保护规则、匹配语法、例外、分类算法** | `plaud-theme-shared/references/sync-reach.md`（唯一事实源） |
...
SyncReach:                # v0.4.0：逐改动项 —— Path / LocaleKey / ChangeKind / MatchedRules /
                          #   ExceptionApplied / EffectiveRule / Classification / OverriddenAtSites / Evidence
                          #   （零命中也要逐路径列出并标 Reachable + MatchedRules: []）
EntrypointCandidates:     # 可选修改入口（模板存值 / schema / 模块代码）+ 各自风险 + 各自的到店结论
