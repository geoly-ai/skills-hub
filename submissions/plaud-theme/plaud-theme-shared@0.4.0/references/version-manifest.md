...
| 项 | 值 |
|---|---|
| 矩阵包版本 | **v0.4.0**（git tag `v0.4.0`） |
| 包名 | `plaud-theme-matrix`（git 仓库 <https://github.com/chovizzz/plaud-theme-matrix>；版本靠 tag 钉，不再有 `-vX.Y.Z` 目录与 zip） |
| 契约版本（`ContractVersion`） | **v0.4.0**（与包版本同步递增。v0.3.2–v0.3.6 都只动包内附带工具 skill 与安装/更新机制，矩阵字段、枚举与路由一字未改；**v0.4.0 是自 v0.3.0 以来第一次真正的契约变更**——见 §1.2） |
...
| **`sync-reach.md` §2 的 15 条保护规则、§2.1 的例外、§3 的匹配语法**（增删或改语义） | 同理：判定只读包内文本。规则变了不发版 = 四端仍按旧规则判「能不能到店」，而人以为已经改了。**注意与 §7 的站点清单分开**：`memory/站点自研代码清单.md` 是**项目运行时状态**，它变了**不需要**发版，直接更新项目侧那份 |

### 1.2 v0.4.0 变更摘要（`SyncReach` —— 改动到不到店）

新事实：PLAUD 用「一套基线 `origin/main` → sync 到 **17 个独立 Shopify 店**」发版，而 **15 条全局保护规则**
圈住的文件与**各店自研代码** sync 都不覆盖各店版本。矩阵此前完全不知道这件事，于是存在一个
**每一道门都绿、而 17 个店一个都没收到改动**的失败形态（`plaud-theme-shared/SKILL.md` §3.1）。

| 项 | v0.3.6 | v0.4.0 |
|---|---|---|
| 新 reference | — | **`sync-reach.md`**（15 条规则、匹配语法、例外、判定算法的唯一事实源） |
| 新 memory seed | — | **`memory-seed/site-custom-code-inventory.seed.md`**（17 站 × 自研文件，快照 **2026-09-03**） |
| §3 Assess 工件 | 15 字段 | **16**（新增 `SyncReach`） |
| §9.1.2 QAIntake | 26 字段 / **六**项材料 | **27** / **七**项材料（新增 `SyncReachStatus`） |
| §9.1.4 ReleaseOps | 28 字段 / 四条硬规则 | **29** / **五**条（新增 `SyncReachCheck`） |
| §8.1 条款分级表 | 11 条 | **12**（新增 #12 🔴「改动必须能到店」；**#8 三层入口整条改写为两级排序**） |
| §9.2 枚举 | — | 新增 `SyncReach[].Classification` / `SyncReach[].ChangeKind` / `SyncReachStatus` / `SyncReachCheck` |
| §3 `InlineLite` | 8 条豁免条件 | **9**（新增：命中 `sync-reach.md` §2 保护规则者不得走 `InlineLite`） |
| §5 QA 准入门 | 六项材料口径 | 七项；读到缺 `SyncReachStatus` 的旧工件判 `Blocked` / `MissingArtifact` |
| §4 Implement / §5 Verify 字段 | 22 / 35 | **不变**（理由见 §1.1 的边界说明与 `handoff-schema.md` §1.1） |

🔴 **这是破坏性变更**：封闭字段集合与封闭枚举都动了，10 个矩阵 skill 必须**同时**升到 `0.4.0`。
部分安装 = 旧 skill 输出 `ContractVersion: v0.3.6`、新 shared / QA 按 `v0.4.0` 校验 → 版本漂移停机；
更糟的是同一份项目 `memory/` 被两套规范处理。`yidian-draft-pr`（§2.1 附带工具 skill）不输出
`ContractVersion`、不参与漂移判定，保持原版本。
...
| **6** | `plaud-theme-qa-intake` | **提测准入关口**（不占阶段轴）：按 DTC §四 校验交付物（v0.4.0 起共**七项**，第七项 `SyncReachStatus` 到店落地方案）、站点清单与包指纹；材料不齐 QA 不启动 | Implement→Verify 过渡 |
...
| **`sync-reach.md`**（v0.4.0 新增） | 基线 → 17 店的分发模型、15 条保护规则全表、§2.1 两条例外、匹配语法（含哪些是报告明示、哪些是本文件的解释约定）、**六个** `Classification` 取值（含 `SiteOverridden`）与八步判定算法、`templates` 文件级 vs `locales` 字段级的粒度区分、站点自研代码的语义与风险方向、与 §8.1 #8「三层入口」的冲突消解 | **改动能不能到店**的全部规则 |
| **`memory-seed/site-custom-code-inventory.seed.md`**（v0.4.0 新增） | 17 站逐站清单 + 20 条 site→file 映射，快照 2026-09-03 | 种子，**不是**运行时事实源（运行时读项目侧 `memory/站点自研代码清单.md`） |
