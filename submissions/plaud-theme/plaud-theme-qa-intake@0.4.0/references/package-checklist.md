...
## 5.1 到店落地方案（`SyncReachStatus`）—— v0.4.0 新增第七项

> 规则、匹配语法、例外与分类算法的唯一事实源是 `plaud-theme-shared/references/sync-reach.md`。本节不复制规则。

PLAUD 是「一套基线 `origin/main` → sync 到 **17 个独立 Shopify 店**」。**15 条保护规则**圈住的文件
（`templates/**/*.json`、`locales/*.json`、`sections/*.json`、`config/*.json`、一批埋点 snippet、PageFly / Matycube 产物）
sync **不覆盖各店版本**。改了它们：基线变了、各店没变，而 Theme Check / QA / 推站**全部正常绿灯**。

本项与 §5 的分工，一句话：**§5 问「基线里谁受影响」，本项问「这些改动出不出得了基线」。**

| 取值 | 条件 |
|---|---|
| `Complete` | 下面四条**同时**成立 |
| `Incomplete` | 四条任一不成立 |
| `NotApplicable` | `AssessmentRef` 的 `SyncReach` **全部**为 `Reachable`，且附零命中证据（逐路径列出 + `MatchedRules: []` + 写明比对过全部 15 条） |

**`Complete` 的四条**：

1. `AssessmentRef` 里有 `SyncReach`，且**没有任何一条 `Undetermined`**。
2. 每条**需要方案**的项（判据是 `sync-reach.md` **§4.2** 那张表：`Classification` + `ChangeKind` 一起看，**不是**「非 `Reachable` 就要方案」—— `LocaleFieldLevel` + `Added`/`Deleted` 本来就到得了店、不需要方案；另加 §4.2 末行铁律：`OverriddenAtSites` 非空时，对 `OverriddenAtSites ∩ TargetSites` 里的店**永远**要方案，与 `Classification` 无关），在包里有一份**逐店落地方案**文档，逐项三段：

   ```
   路径（+ locale key）→ 为什么到不了店（引 sync-reach.md 的规则号）→ TargetSites 里每个店各自怎么落地（谁 / 何时 / 在哪 / 做什么）+ 出处
   ```

3. 方案覆盖的站点集合 **⊇ `TargetSites`**。少一个店即 `Incomplete`。
4. 方案文档在 `PackageRootRef` **之下**（因而进 `PackageFingerprint`）。

**判 `Incomplete` 的情形**：

- 只写"这些文件各店自己维护"，没有逐店的动作与出处 —— 与 §5 的"显示正常"同一类不可复核表述
- 方案只覆盖了 `TargetSites` 的一部分
- 方案挂在包外的链接上（内容可被替换而指纹察觉不到，与 §3.1 要求旧用例清单必须 `Local(<相对路径>)` 同一个理由）
- `SyncReach` 里有 `Undetermined` —— 这说明 Assess 本就该 `ReadyForImplement: No`，退回 `plaud-theme-impact`

**两条没有 §3 工件的承载路径**（v0.4.0 收口，否则两类合法提测结构性死锁）：

| 情形 | 怎么判 |
|---|---|
| `ReconMode: InlineLite` | §3 的 InlineLite 第 9 条要求**不命中 15 条保护规则** → **豁免成立 ⇔ 零命中** → `NotApplicable`，证据是实现工件里那条第 9 项的逐路径核查结果（含 `MatchedRules: []`）。缺该核查结果 = 豁免没自证 → `Incomplete` |
| 集成提测包 | 顺 `IntegrationOf` 逐块取 `AssessmentRef.SyncReach`，**按三元组 `(Path, LocaleKey, ChangeKind)` 求并集**后按上面四条判。任一成员块含 `Undetermined` → `Incomplete`。🔴 成员块是 `ReconMode: InlineLite` 时它本来就没有 §3 工件 —— 用它 InlineLite 第 9 项的逐路径核查结果当作那份 `SyncReach` 并入（**只有「缺 `SyncReach` 且不是 InlineLite」才判 `Incomplete`**）。🔴 同一 `(Path, LocaleKey)` 被两块判成不同 `ChangeKind` 或不同 `Classification` → **停机**（冲突只有集成者能定），且**不得**往 `IntegrationOf` 加字段 |

> 🔴 **本项最容易被糊弄过去。** `AssessmentRef` 里已经躺着一份判得很细的 `SyncReach`，顺手把本项也填 `Complete`
> 毫无阻力。但本项要的是**方案**不是事实。唯一可机械核的判据只有一条：
> **包目录里存在那份文档，且它列出的站点集合 ⊇ `TargetSites`。** 文档不存在就是 `Incomplete`。

> 🔴 **「本来就打算让各店自己本地化」不是 `NotApplicable`。** 那是一份完全合法的**落地方案**
> （写清"各店保持自己的版本，本次不需要任何店做动作，出处：<运营确认>"即可），要写进文档、判 `Complete`。
> `NotApplicable` 只对**零命中**成立。
...
**下表是单块提测包的判定。集成提测包（`ChangeSetId: N/A(Integration)`）**在此之上**还要满足 §7 的全部追加条件，七项全绿也不够 —— 见 §7.3 / §7.4 / §7.5 / §7.6。**

| `SubmissionPackageStatus` | 条件 |
|---|---|
| `Complete` | **七项 Status** 全为 `Complete`（`ConfigurationGuideStatus` / **`SyncReachStatus`** / `ReworkDeltaStatus` 可为 `NotApplicable` + 理由），且 `PreviewManifestStatus: Complete`（两条链接实测可访问）**；集成包另须 §7 全部条件成立** |
| `Incomplete` | 其余任何情况 |
...
| **B. 该块根本没有提测包**（`ChangeSetStatus` 与用户手上都拿不到该块的 `SubmissionId`） | **停机，不出契约块**。27 key 契约要求 `IntegrationOf` 每项都有 `SubmissionId`，而这个值**不存在**。`BlockingGaps` 写进正文，指名是哪一块从未提测，要它**先补一次单块提测**拿到真 ID 与 `Complete` 的包 |
...
1. 对 `IntegrationOf` 里**每一块**，用它的 `SubmissionId` 找回那份 `QAIntake` 工件，读它的 **`PackageRootRef`**（这是 27 key 里真实存在的字段）；
...
> 📌 **为什么只能做到这一步**：`QAIntake` 的 27 key 里**没有材料清单/inventory 字段**，`PackageFingerprint` 也**反推不出**文件清单（它是单向 hash）。所以"成员材料 → 集成材料"的逐条映射在本版没有受契约绑定的承载物。**已上报 shared**。
