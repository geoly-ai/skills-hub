...
**准入门（Step 0，最早）**：四查——intake 的 `ChangeSetId` + **三元组**（`ObjectFormat` / `ThemeTreeOid` / `ChangeSetScopeFingerprint`）与 Implement 工件逐字比对（防重放）、重算 `PackageFingerprint`（防准入后替换）、`SubmissionPackageStatus`（v0.4.0 起是**七项**材料口径，第七项 `SyncReachStatus`；工件里**缺这个 key** = 旧版本包 → `Blocked` / `MissingArtifact`，**不得**当成 `NotApplicable`）、**测试集三行的绑定自洽**（`TestSetTrace` / `PreviousAcceptedTestSetTrace` / `TestSetMigrationRef`，含 `N/A` 的双向约束与迁移清单自洽复核）。
