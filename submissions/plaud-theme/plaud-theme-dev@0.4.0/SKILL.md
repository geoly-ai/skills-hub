...
- 用了豁免就必须写 `ReconMode: InlineLite` + **白名单归类 + §3 九个条件逐条对照（v0.4.0 起含 sync-reach 保护规则未命中） + 七维证据命令原文 + **第 9 条（sync-reach 保护规则未命中）的逐路径核查结果****。写不满 → 说明它本来就不该走豁免。
...
**Path A 最常踩的一脚**：改 `templates/*.json` 的模板存值。它命中规则 #15 —— 只有**基线新增**的模板
能同步过去，各店**已本地化**的模板一律不覆盖。而"改模板存值"恰恰是三层入口里被优先推荐的那一层。
