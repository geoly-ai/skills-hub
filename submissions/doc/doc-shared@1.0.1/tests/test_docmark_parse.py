...
GOLDEN = os.environ.get('DOC_GOLDEN') or os.path.expanduser('~/workspace/docs/doc-skill-platform-2026-09-15/golden')
LEGACY_RENDER = os.environ.get('LEGACY_RENDER') or os.path.join(os.path.dirname(ROOT), 'presales-publish', 'scripts', 'render.py')
...
def run(verbose=True):
    ...
    if os.path.exists(LEGACY_RENDER):
        ...
        print('SKIP docmark：旧渲染器不存在（LEGACY_RENDER=' + LEGACY_RENDER + '），跳过 include 兼容比对')
    ...
    if os.path.isdir(GOLDEN):
        ...
        print('SKIP docmark：golden 目录不存在（DOC_GOLDEN=' + GOLDEN + '），跳过旧文档兼容块')
