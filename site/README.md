...
装机量、调用量、下载量 —— **本站点的数据只来自签名过的快照，而快照里没有这类字段**。
本站点不读取、也不接入任何埋点聚合。
所以模型里连字段都不留，页面上也不留"即将上线"的占位。

🔴 **这条的依据别写成「埋点收不到数据」。** 早先就是那么写的（「埋点是纯本地的，
没有内置默认端点」），结果 2026-09-01 上报链默认开之后那句话当场变成假的 ——
而它印在每一页上。免责声明的依据只能是**本站点自己**的事实，
不能是别处的实现细节，否则别处一改这里就成了谎。
