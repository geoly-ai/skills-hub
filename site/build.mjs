 ...
 */
// 🔴 **这句话的理由改过一次，别再改回去。**
//    原文写的是「埋点是纯本地的，没有内置默认端点」—— 那在 2026-09-01
//    埋点上报链合并进来的**同一刻**就变成假话了（规格 v5 §4.2：上报默认开、
//    有内置默认端点）。而它印在**每一页**上。
//    ⚠️ 教训：这句免责声明的依据不该是「埋点收不到数据」，那是**别处**的实现
//    细节，别处一改这里就成了谎。依据只能是**本站点自己**的事实 ——
//    它不读、也不展示任何埋点聚合。那一条不随埋点怎么演进而改变。
const NO_USAGE_STATEMENT = '本站点不展示任何形式的使用情况指标。'
  + '本站点的数据只来自签名过的快照，它不读取、也不接入任何埋点聚合；'
  + '而快照里根本没有这类字段。'
