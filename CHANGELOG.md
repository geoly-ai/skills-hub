...
**批量安装不再给每个制品记同一段整批耗时。** 一次 `install` / `update` 在同一个 target 上
是一个事务，耗时只有一个；此前却给这次请求的每个制品各记一条带这个耗时的埋点。
一条命令装 N 个 skill（或 `--all`）时，这段耗时被算了 N 遍，服务端按制品版本算的
安装耗时分位数因此按制品数加权 —— 0.3.8「明确没做到」里的最后一条。

现在只有「这个 target 这次只请求了一个制品」时事件才带耗时，分位数的口径就是
「单个制品装一次要多久」。批量安装的事件照记，只是不带耗时、不进分位数。

- 装 pack 不受影响：埋点按请求的 root 记，pack 只有它自己一条，照常带耗时
- 0.3.8 及更早版本上报的事件仍是旧口径，会在库里留到保留期到期
