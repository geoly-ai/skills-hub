...
**自动上报几乎从不成功。** `install` 成功后的自动上报超时原先是 1 秒，而实测从国内经代理到端点
一次往返要 1.3–1.7 秒 —— 请求在 1 秒处被中断，事件留在本地，并且要等 24 小时才会再试。
结果是装了很多次也几乎没有数据回流。现在：

- 超时放宽到 **3 秒**
- 发送失败时 **1 小时后可再试**，不再压满 24 小时（成功时仍是 24 小时最多一次）
- 端点挂着时不会每次 `install` 都去撞：1 小时内只试一次

⚠️ 代价：`install` 收尾最坏多等 3 秒（原先 1 秒）。不想等可以 `GEOLY_TELEMETRY_UPLOAD=0`。

**上报的 CLI 版本号一直是 `0.0.0-m1`。** 事件里的 `cli` 字段取的是环境变量、缺省回落到一个
写死的字面量，而从 npm 装下来运行时没有人设那个变量。现在与版本门同源，直接读 `package.json`。

### 新增

**`skills-hub telemetry delete` 现在会先申请服务端删除。** 此前它只清本机。
现在的顺序是：先向服务端证明「这些身份记录是这台机器发的」（本机私钥签名、挑战-响应），
服务端删掉身份记录并剥掉相关事件上的 `install_id`，然后才清本机。

- `--offline` / `GEOLY_TELEMETRY=0` / `GEOLY_TELEMETRY_UPLOAD=0` 下**不出网**，私钥保留，退出码非零
- 服务端删除失败时**私钥保留**（否则那批记录再也证明不了是谁的），退出码非零
- 不再删除「关闭身份」的设置 —— 先 `telemetry off` 再 `telemetry delete` 的人，删完仍是关的

**身份三项（登录名 / 主机名 / 来源 IP）的采集开关**，**默认关**：
要 `GEOLY_TELEMETRY_IDENTITY=on` 并且看过一次单独的告知才会采；`skills-hub telemetry off`
只关它、匿名计数照发。服务端的身份摄入同样默认关闭。

### 明确没做到（照例逐条列）

- **私钥丢了就无法按所有权删除**：只能等身份记录 90 天到期。CLI 会如实说「无法证明」。
- **自动上报仍是在 `install` 收尾时同步等待**，不是后台发送；超时只限制网络那一段，
  认领节流戳与上报锁等本地 I/O 不在超时之内。
- **耗时分位数按制品数加权**：同一个 target 下的每个制品记的是整批耗时。
