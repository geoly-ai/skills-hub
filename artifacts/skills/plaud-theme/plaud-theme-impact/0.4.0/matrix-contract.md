...
| **项目侧 `memory/站点自研代码清单.md`**（v0.4.0） | 17 个店各自的自研代码 + 快照 `AsOf`。判 `SyncReach` 的站点自研代码那一半要它。缺失或 `AsOf` 过旧 → `Undetermined` + 停机；种子在 `plaud-theme-shared/references/memory-seed/`，**只在用户确认首次接入时**才可复制 |
...
| C | `LegacyImpact` | — |
| 任意 | `InlineLite` | 仅当 **9** 条豁免条件全部满足；拿不准就不是。🔴 v0.4.0 新增第 9 条：**命中 `sync-reach.md` §2 保护规则的改动一律不得走 `InlineLite`**（§2.1 例外内的不算命中） |
