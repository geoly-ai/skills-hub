...
description: PLAUD Shopify 主题矩阵的契约层（order 0）：两轴状态机、handoff schema、ChangeSetId 绑定、 交付权归属、Stop-don't-guess 停机规则、全路径红线、视觉与 UX 基线索引、版本清单。 矩阵内任何 skill 在开工前都必须先读本 skill；全流程任务必须过本层契约。 本 skill 不实现任何主题改动、不做 bug 修复、不写 section、不做 UX 迁移、不做验收—— 它只定义规则与交接格式。用户不需要直接调用本 skill；由 plaud-theme-orchestrator / plaud-theme-impact / plaud-theme-dev / plaud-theme-section-build / plaud-theme-ux-migration / plaud-theme-qa-intake / plaud-theme-qa / plaud-theme-feedback-triage / plaud-theme-release-ops 引用。若用户直接问"矩阵怎么衔接""handoff 字段是什么""为什么必须过 QA"，可直接读本 skill 回答。
---
...
> 🔴 **v0.3.0 起 `ReadyForDelivery: Yes` 有两个合法来源**：`QAScope: SingleChangeSet`（单块直发）与 `QAScope: Integration`（多块同批）。真正的门不是 scope，而是 handoff-schema §2.11 的**两层九条**——QA 侧四条当场可验，release 侧五条由 `plaud-theme-release-ops` 守：`ReleaseSourceTreeOid == VerifiedThemeTreeOid` + `ReleaseDeclaredDiffCheck: Passed` + `PushCommandCompliance: Compliant` + 推站紧前复算 + 🔴 **v0.4.0 新增的 `SyncReachCheck: Passed`（这棵树到不到店）**。多块合并后没有任何单块 QA 持有那棵合并树的 oid，于是**结构上必然**要求一份集成 QA。
...
### 6. 改动进了基线 ≠ 改动到了店（v0.4.0 新增）

PLAUD 是「一套基线 `origin/main` → sync 到 **17 个独立 Shopify 店**」。其中**两类文件 sync 不覆盖各店版本**：
15 条全局保护规则圈住的文件（`templates/**/*.json`、`locales/*.json`、`sections/*.json`、`config/*.json`、
一批埋点 snippet、PageFly / Matycube 产物），以及各店自己写的**站点自研代码**。

> 🔴 **这是矩阵里唯一一个「每一道门都绿、而业务上什么都没发生」的失败形态**：
> 改动进了基线、Theme Check 过、QA 过、推站成功、`RemoteVerifyResult: Matched`、`PerSitePushResult` 全 `Succeeded`
> —— 而 17 个店一个都没收到。现存每一道门验的都是「基线这棵树对不对」，**没有一道验「这棵树到不到店」**。

判定链：`plaud-theme-impact` 的 `SyncReach`（事实）→ `plaud-theme-qa-intake` 的 `SyncReachStatus`（逐店落地方案齐不齐）
→ `plaud-theme-release-ops` 的 `SyncReachCheck`（推站前重判）。规则、匹配语法、判定算法的唯一事实源是
`references/sync-reach.md`；红线是 handoff-schema §8.1 第 12 条。

⚠️ 数据是 **2026-09-03** 导出的快照，**会过时**；per-site 的自研代码清单是项目运行时状态，在项目侧
`memory/站点自研代码清单.md`，不在本包。
...
| **改动会写入 `templates/` `locales/` `sections/*.json` `config/` `snippets/` 下任何文件之前（必读）**，以及选修改入口之前 | `references/sync-reach.md` |
...
| **`memory/站点自研代码清单.md`** | **v0.4.0 新增**。17 个店各自的自研代码（基线里没有、各店自己写的 `.liquid`/`.js`/`.css`）+ 配置类原创计数 + 快照日期 `AsOf`。判「基线新建的文件会不会与某店同名文件撞车」「改共享文件时哪几个店的实际影响面大于评估值」时读它 | 用户 / 运营（从报告导出）；矩阵**只读不写** |
...
| **`站点自研代码清单.md`** | **同一条规则，只是种子换个位置**（v0.4.0）：**默认停机**。仅当用户明确确认"首次接入"后，才可从 **`plaud-theme-shared/references/memory-seed/site-custom-code-inventory.seed.md`** 复制初始化。🔴 复制之后**必须**核它的 `AsOf`（种子是 **2026-09-03** 的一次性快照）：判定依赖的事实比 `AsOf` 新时，按 `sync-reach.md` §4.1 取 `Undetermined` 并要求重新导出报告，**不要拿旧快照当现状**。种子文件本身**永远不是**运行时事实源 |
