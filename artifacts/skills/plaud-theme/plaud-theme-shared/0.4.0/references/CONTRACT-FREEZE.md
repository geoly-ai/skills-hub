...
>
> 🔴 **v0.4.0 起本文件已是历史记录，且其字段计数全部过期。** v0.4.0 的 `SyncReach` 变更**不在**本文件覆盖范围内：
> §3 现为 16 字段、§9.1.2 为 27、§9.1.4 为 29，`SyncReach` / `SyncReachStatus` / `SyncReachCheck` 三个字段与
> §8.1 第 12 条红线本文件一概没有。**不要拿本文件的字段清单去核 v0.4.0 的工件**——那会把新字段判成结构违规。
> v0.4.0 的施工视图见 `version-manifest.md` §1.2，规则见 `sync-reach.md`。
