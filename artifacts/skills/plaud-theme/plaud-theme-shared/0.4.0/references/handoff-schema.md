...
| **这批改动到得了各店** | 🔴 **v0.4.0 新增**。`plaud-theme-impact` 的 `SyncReach`（事实）→ `plaud-theme-qa-intake` 的 `SyncReachStatus`（材料）→ `plaud-theme-release-ops` 的 `SyncReachCheck`（推站前重判）。见 `sync-reach.md` |

四者正交，任何一个都不能替代另一个。QA 通过后仍可能被 PM 判为交付缺陷（例如与 Figma 不一致——这是 QA 不检查的维度）。

> 🔴 **为什么「到不到店」不做成 §5 的一个技术检查项**（v0.4.0 决策，明写以免下一轮又被提出来）：
> `ReadyForDelivery` 的定义是「**这棵树**技术上可以交给 release 使用」，判的是树的内容；
> 而到不到店是**分发侧**的事实，与树的质量无关，且依赖 `TargetSites`——那是 QA 阶段还没定死的东西。
> 把它塞进 §5 会让 QA 去替 release-ops 做发版合法性判定，正是本节要划开的那条边界。
>
> **但这不等于 QA 可以对它一无所知**：`SyncReachStatus` 是 §9.1.2 的第七项材料，
> 它 `Incomplete` 时 `SubmissionPackageStatus` 就是 `Incomplete`，QA 的 Step 0 准入门直接 `Blocked`、零验证项执行。
> 也就是说 QA **不判**到不到店，但**没判过到不到店的东西进不了 QA**。
...
9. 🔴 **v0.4.0 新增**：`SyncReachCheck: Passed`（§9.1.4）——本次改动**到得了目标店**，或每条到不了的项都有覆盖全部 `TargetSites` 的逐店落地方案。
   `Failed` / `Blocked` 都不得放行（`Blocked` 是「该验但验不了」，与第 6 条同一条纪律）。

> 🔴 **第 9 条与第 5–8 条验的不是同一件事。** 5–8 全部在问「**基线这棵树**对不对、推的是不是它」；
> 第 9 条问「**这棵树到不到店**」。sync 对 15 条保护路径保留各店版本，所以 5–8 可以全绿、推送正常完成、
> `RemoteVerifyResult: Matched`、`PerSitePushResult` 全 `Succeeded`，而 17 个店一个都没收到改动。
> 少了第 9 条，本节这个「不变量」只覆盖到基线为止，覆盖不到业务上「上线了没有」。规则见 `sync-reach.md`。
...
SyncReach:               # 🔴 v0.4.0 新增。本次改动**能不能通过 baseline → 各店 sync 到店**。
                         #   规则表（15 条保护 glob）、例外、匹配语法、判定算法的**唯一事实源**是
                         #   `references/sync-reach.md`；本字段只规定形态，不复制规则。
                         #   逐**改动项**一条（不是逐文件一个标量）：
                         #     - Path:             逐字路径，与 §4 ModifiedFiles 同一串写法
                         #       LocaleKey:        仅 locales/*.json：点分 JSON key 全路径（见 sync-reach §5.3）；
                         #                         其余填 N/A
                         #       ChangeKind:       Added | Modified | Deleted
                         #                         （重命名拆成 Deleted(旧) + Added(新) 两条，见 sync-reach §3）
                         #       MatchedRules:     命中的保护规则原文，逐条全列；零命中填 []
                         #       ExceptionApplied: 命中的例外规则原文（sync-reach §2.1）；无则 None
                         #       EffectiveRule:    例外优先后最终生效的那一条；MatchedRules 为 [] 时填 None
                         #       Classification:   Reachable | NotReachable | NewTemplateOnly
                         #                         | LocaleFieldLevel | SiteOverridden | Undetermined
                         #       OverriddenAtSites: 被哪几个店的同名自研文件覆盖（sync-reach §5.1 第 6 步）；无碰撞填 N/A。
                         #                         🔴 **不是「仅 SiteOverridden 才填」**（v0.4.0 第三轮评审纠正）：
                         #                         碰撞与 glob 结论是两件独立的事，可以同时成立。glob 判出
                         #                         NotReachable / NewTemplateOnly / LocaleFieldLevel 时 Classification
                         #                         保留 glob 结论（§5.1 第 7 步），但**碰撞照记**。
                         #                         🔴 OverriddenAtSites 非空 → 对 OverriddenAtSites ∩ TargetSites 里的
                         #                         每个店都必须有落地方案，**与 Classification 是什么无关**
                         #                         （sync-reach §4.2 末行铁律）—— 只看 Classification 会漏掉
                         #                         「LocaleFieldLevel + Added 到得了店、但碰撞店收不到」这一类。
                         #       Evidence:         判定依据（命令原文 / sync-reach.md 节号 / memory 清单的 AsOf）
                         #   🔴 零命中也要逐路径列出并标 Reachable + MatchedRules: []（sync-reach §5.4）——
                         #      一句「本次不涉及受保护路径」与「没查过」不可区分
                         #   🔴 **覆盖等式**（sync-reach §5.3.1）：本字段出现的可发布路径集合必须**恰好等于**
                         #      本次计划写入集的可发布路径集合（提测时对 §4 ModifiedFiles、发版时对声明并集）。
                         #      少一条 = 有路径根本没判过，而下游会把「没出现」读成「所有项都 Reachable」→
                         #      NotApplicable → Passed，三道门全绿而真正会出事的路径从没进过任何工件。
                         #      多一条 = 拿的是上一轮的 SyncReach。两个方向都阻断。
                         #      locales 的 key 集合须覆盖该文件本次全部变更的 key；重命名须是两条。
                         #   🔴 任一条 Classification: Undetermined → ReadyForImplement 必须为 No
                         #      且 BlockingGaps 非空写明缺什么（sync-reach §4.1）
                         #   🔴 本字段判「这条路径能不能过 sync」，**不判「要推哪几个站」** ——
                         #      站点清单仍由 plaud-theme-qa-intake 在 §9.1.2 填，本 skill 不得推断
                         #   🔴 **本字段没有 N/A(ReadOnly) 这个取值**：零改动只读任务根本不产出 §3 工件
                         #      （它由实现 skill 内联出 §4 + ReadOnlyProof，AssessmentRef 填 N/A(ReadOnly)），
                         #      所以那个状态在本字段里不可达。为一个不可达的状态留取值 = 给「其实没判」
                         #      开一条看起来合法的出口。
EntrypointCandidates:    # 可选修改入口（模板存值 / schema / 模块代码）+ 各自风险。
                         #   🔴 v0.4.0：排序改为两级 —— 先按 SyncReach 筛掉不可达入口，
                         #   再在可达入口里按 模板存值 → schema → 模块代码 排（§8.1 #8 / sync-reach §8）
...
> **本块共 16 个 key**（v0.3.6 为 15），顺序即上表顺序，是封闭集合。
...
>
> 📎 **`SyncReach` 不违反这一条，它问的是另一个问题。** 「要推哪几个店」是**决策**（运营给，有出处）；
> 「这个文件能不能过 sync」是**规则匹配的事实**（15 条 glob 逐条比对，与推给谁无关）。两者正交，别互相代入。

> 🔴 **多店场景下 `TheoreticalReferences` / `ActualAffectedInstances` 有系统性下界偏差**（v0.4.0 补）：
> 它们只扫**基线仓库**，而 GLOBAL / JP / LATAM / TW / US 各有基线里根本不存在的**站点自研代码**
> （`sync-reach.md` §7）。改共享 snippet / 全局 CSS / token、或删除重命名 snippet 时，那几个店的实际影响面 **≥** 评估值。
> 这一点写进 `SharedPropagation` 的说明文字，**不得**默认「基线里没引用 = 没人用」。清单在项目侧
> `memory/站点自研代码清单.md`（种子见 `references/memory-seed/`），拿不到就按 `Undetermined` 停机。
...
- `InlineLite` — 仅限**全部**满足：改动 ≤ 1 个文件、该文件无其它引用方、非共享 snippet / 非全局 CSS / 非 token / 非 build 产物、不改 schema、不改模板存值、**且该文件不命中 `sync-reach.md` §2 的 15 条保护规则（落在 §2.1 例外内的不算命中）**。此时实现 skill 可自行内联完成评估，但仍须在 HandoffContract 写明 `ReconMode: InlineLite` 与豁免理由。**拿不准就不是 InlineLite。**

  > 🔴 **v0.4.0 新增的最后一条不是可选项**：`InlineLite` 原有的豁免条件全是「这个文件在**仓库内**没有别的引用方」这一类判据，而到不到店是**仓库外**的分发事实——仓库里怎么查都查不出来。一个只改 `snippets/ga4-push.liquid` 的单文件改动完全满足旧条件，却是 17 个店一个都收不到的典型形态。命中保护规则即须过 `plaud-theme-impact` 出 `SyncReach`。
...
ReadyForDelivery:        # Yes | No —— 见 §2.11。QA 侧四条 + release 侧五条（v0.4.0 起含 SyncReachCheck）。
                         #   两种 QAScope 都可以取 Yes
```
...
1. 有 `SubmissionId` 且 `SubmissionPackageStatus: Complete` → `Accepted` + `QAAdmissionReason: Normal`，继续走 Step 1 指纹校验。
   🔴 **v0.4.0 补：`SubmissionPackageStatus: Complete` 现在是七项材料全 `Complete`/`NotApplicable`，第七项是 `SyncReachStatus`**（§9.1.2）。QA **不得**沿用旧的「六项」口径去读一份新工件——那样 qa-intake 已经填了 `SyncReachStatus: Incomplete`，QA 却照样 `Accepted`，这道门等于没接上线。读到一份**缺 `SyncReachStatus` key** 的 §9.1.2 工件时，那是 v0.3.6 及以前的旧工件：`QAAdmissionStatus: Blocked` + `QAAdmissionReason: MissingArtifact`，要求按当前 `ContractVersion` 重出提测包，**不得**当成 `NotApplicable` 放行。
...
- 🔴 **v0.4.0**：`SyncReach` 里出现任一 `Undetermined`（匹配语法的解释约定边界、`BaseHeadSha` 不可解析、
  `memory/站点自研代码清单.md` 缺失或过期、报告未覆盖的路径形态）→ 停，要材料。**不得**先按"应该能到店"实现
  （判据见 `sync-reach.md` §4.1）
- 🔴 **v0.4.0**：三层入口里**没有任何一个**能到达目标店 → 停，交运营决定走逐店手工落地还是改需求。
  **不得**自行选一个不可达入口做完然后宣布交付（`sync-reach.md` §8）
...
> **§8.1 的 12 条里目前没有任何一条可走 `ApprovedException`。** 尤其：
>
> - **第 12 条（改动必须能到店）是 🔴，不可批准豁免**，理由与第 10 条同构：拿到书面批准也不会让改动真的同步到店，批准改变不了分发引擎的行为。它要求的本来就只是「判过 + 每条不可达的项有逐店落地方案」，这是能做到的事，不存在需要豁免的情形。批准链接只能作为落地方案的 `ScopeSourceRef`，不能替代方案本身。
>
> 其余：
>
...
| 8 | **先按「能不能到目标店」筛掉不可达入口；再在可达入口里按 模板存值 → schema → 模块代码 排序** | 🟠 **EvidenceBased** | 🔴 **v0.4.0 改写，不是加注**：原条款只有第二级。多店基线下第一档「模板存值」恰好是 sync 过不去的那一档（`templates/**/*.json` = `NewTemplateOnly`、`config/*.json` = `NotReachable`），照原顺序做会做出一个改动完成、验收通过、推站成功而 17 个店一个都没变的交付。判定顺序与三种粒度见 `sync-reach.md` §8。偏离第二级顺序时，仍用**既有的 `OptionsConsidered`（§4）**说明 + 引用 `AssessmentRef`；因第一级筛掉而落到较低层的，把 `SyncReach` 的判定写进 `OptionsConsidered` 即构成完整论证，**不需要额外审批**。**不新增 `EntrypointRationale` 字段**——那会形成第二个事实源 |
...
| 12 | **改动必须能到店**：命中保护清单的路径，必须有逐店落地方案 | 🔴 | **v0.4.0 新增。** 基线里改了、各店收不到 = 这次交付在业务上等于没发生，而现存每一道门验的都是「基线这棵树对不对」，没有一道会报错。判定链：`plaud-theme-impact` 出 `SyncReach`（事实，§3）→ `plaud-theme-qa-intake` 判 `SyncReachStatus`（材料齐不齐，§9.1.2）→ `plaud-theme-release-ops` 推站前重判 `SyncReachCheck`（§9.1.4）。规则、匹配语法与判定算法的唯一事实源是 `sync-reach.md`。🔴 **本条红线要求的不是「必须可达」**——各店自行本地化模板是合法的产品形态；要求的是**判过、并且对每条不可达的项有一个有出处的逐店落地方案**。没判过、或判了却没有方案，才是违规 |

> 📎 **第 12 条的出处不是 DTC。** §8.1 的 #1–#11 源自《DTC 开发交付标准 v1.0》§三；#12 源自
> 《PLAUD DTC 保护与站点自研代码报告》（2026-09-03 导出）。放进这张表是因为它与 #3 / #4 同族——
> 都是**发版与推站**侧的约束、都由 `plaud-theme-release-ops` 守最后一道。分级同样适用三档定义。
...
SyncReachStatus:          # 🔴 v0.4.0 新增，第七项材料。Complete | Incomplete | NotApplicable
                          #   判的是「到店落地方案齐不齐」，不是「能不能到店」（后者是 AssessmentRef 里的事实）。
                          #   Complete 需**同时**满足四条：
                          #     ① AssessmentRef 的 SyncReach 存在且**无任何 Undetermined**；
                          #     ② 每条**需要方案**的项，在提测包目录里有对应的**逐店落地方案**文档 ——
                          #        🔴 判据是 sync-reach.md §4.2 那张表（Classification + ChangeKind 一起看），
                          #        **不是**「非 Reachable 就要方案」：LocaleFieldLevel + Added/Deleted 本来就
                          #        到得了店、不需要方案，照旧判据会让这类合法改动既填不了 NotApplicable
                          #        （不是全 Reachable）、又交不出方案（没有可写的落地动作）→ 结构性死锁。
                          #        另加 §4.2 末行铁律：OverriddenAtSites 非空时，对 OverriddenAtSites ∩ TargetSites
                          #        里的店**永远**要方案，与 Classification 是什么无关 ——
                          #        逐项写明：本工件 TargetSites 里的**每一个**站点各自怎么落地
                          #        （谁在什么时候、在哪个店的后台或分支上做什么），并带出处；
                          #     ③ 该方案覆盖的站点集合 ⊇ 本工件的 TargetSites（少一个店即 Incomplete）；
                          #     ④ 方案文档在 PackageRootRef 之下，因而被 PackageFingerprint 覆盖 ——
                          #        🔴 放在包外（只给个链接）不算数：那样材料可以在准入通过之后被替换，
                          #        而包指纹察觉不到，这道门就退化成一次性的口头承诺
                          #   NotApplicable 仅当 SyncReach **全部**为 Reachable，且附零命中证据
                          #   （SiteOverridden 与 NotReachable 同等对待，都属于「非 Reachable」）
                          #   🔴 每条 SiteOverridden 的 OverriddenAtSites 必须非空且逐店列出，方案须覆盖那几个店；
                          #      标了 SiteOverridden 却填 N/A 是自相矛盾 → Incomplete
                          #   🔴 **两条没有 §3 工件的承载路径**（v0.4.0 收口，否则两类合法提测结构性死锁）：
                          #     · ReconMode: InlineLite（AssessmentRef 填 InlineLite，没有 §3 工件）——
                          #       §3 的 InlineLite 第 9 条本来就要求「不命中 15 条保护规则」，所以豁免成立
                          #       **等价于**零命中。此时 SyncReachStatus 取 NotApplicable，证据是实现工件里
                          #       那条第 9 项核查结果（逐路径 + MatchedRules: []）。🔴 但**豁免不成立就没有
                          #       这条路**：一旦命中保护规则，InlineLite 本身就非法，退回 plaud-theme-impact。
                          #       🔴 qa-intake **必须自己拿 ModifiedFiles 重跑一遍 15 条 glob 核对那份自证**：
                          #       只采信实现方声称的「零命中」，等于让「谎称 InlineLite + 伪造一行
                          #       MatchedRules: []」同时绕过 impact 与 qa-intake 两道门。重跑 = 比对 15 行规则。
                          #       🔴 **还要比对 memory/站点自研代码清单.md**：InlineLite 第 9 条只管 15 条 glob，
                          #       管不到站点自研代码碰撞——「零 glob 命中但与某店自研文件同名」的路径完全满足
                          #       InlineLite 九条，却对那些店到不了。碰撞非空 → 要方案，不得填 NotApplicable。
                          #     · ChangeSetId: N/A(Integration)（集成提测包，顶层没有 AssessmentRef）——
                          #       取 IntegrationOf 里**每一块**各自 §4 工件的 AssessmentRef，把各块 SyncReach
                          #       按三元组求并集后判定；任一成员块含 Undetermined → Incomplete。
                          #       🔴 成员块是 ReconMode: InlineLite 时它**本来就没有** §3 工件：用该块实现工件里
                          #       InlineLite 第 9 项的逐路径核查结果当作它那份 SyncReach（全 Reachable +
                          #       MatchedRules: []）并入，qa-intake 同样对它重跑 glob 与站点自研清单核对。
                          #       只有「缺 SyncReach 且不是 InlineLite」才判 Incomplete —— 把合法 InlineLite
                          #       成员判成缺失，就是那个结构性死锁本身。
                          #       🔴 求并的**键是三元组 (Path, LocaleKey, ChangeKind)**，不是 Path ——
                          #       同一 Classification 可以对应不同 ChangeKind（templates 的 Added/Modified 都是
                          #       NewTemplateOnly；locales 三种 ChangeKind 都是 LocaleFieldLevel），只比
                          #       Classification 根本发现不了冲突。**同一 (Path, LocaleKey) 被两块判成不同
                          #       ChangeKind 或不同 Classification → 停机**：合并后到底算新增还是修改，
                          #       只有集成者说得清，自己挑一边等于替他做了个查不到依据的决定。
                          #       🔴 不得为此往 IntegrationOf 加字段（它只有 ChangeSetId + SubmissionId 两段，
                          #       27-key 闭集也不容第二个引用）——聚合结果写在正文，字段只留最终标量。
                          #   （sync-reach.md §5.4：逐路径列出、MatchedRules: []、写明比对过全部 15 条）。
                          #   🔴 「本次不涉及受保护路径」这类整句结论**不构成**零命中证据 —— 它与没查过不可区分
                          #   规则、匹配语法、分类算法的唯一事实源：references/sync-reach.md
ReworkDeltaStatus:        # Complete | Incomplete | NotApplicable（非返工轮次填 NotApplicable）
SubmissionPackageStatus:  # Complete | Incomplete —— 上述**七项 Status** 全 Complete/NotApplicable 才为 Complete
                          #   （v0.3.6 为六项；v0.4.0 加入 SyncReachStatus）
BlockingGaps:             # 缺哪份材料、缺什么字段，逐项写清
NextRequiredSkill: plaud-theme-qa
```

> **本块共 27 个 key**（v0.3.6 为 26，v0.2.3 为 23），顺序即上表顺序，是封闭集合。
...
**七项材料的验收标准**（#1–#6 对应 DTC §四；#7 `SyncReachStatus` 是 v0.4.0 新增，出处见 §8.1 #12）：
...
| `SyncReachStatus` | 见上方字段注释的四条。🔴 **最容易糊弄过去的一项**：`AssessmentRef` 里已经有 `SyncReach` 这份事实了，很容易顺手把本项也填 `Complete`——但本项要的是**方案**不是事实。判据只有一条可机械核的：包目录里**存在**那份逐店落地方案文档，且它列出的站点集合 ⊇ `TargetSites`。文档不存在就是 `Incomplete`，不管 `SyncReach` 判得多细 |
...
SyncReachCheck:           # 🔴 v0.4.0 新增。Passed | Failed | Blocked（**刻意没有 NotApplicable**：
                          #   发版必然有 ChangeSet，「一条保护规则都没命中」的正确取值是 Passed + 零命中证据，
                          #   填 NotApplicable 等于把「查过、没命中」和「没查」写成同一个值）
                          #   **推站前重判一次**，不是抄 QAIntake 的 SyncReachStatus：
                          #     · 那一份判的是提测时的材料齐不齐；这一份判的是**本次实际 TargetSites**
                          #       与**当前**的 memory/站点自研代码清单.md 下，逐店结论还成不成立；
                          #     · 提测到发版之间 TargetSites 可能已经变过（二次确认就是为了这个），
                          #       多出来的站点在提测时的落地方案里没有覆盖 → Failed
                          #   期望输入 = IncludedInThisPush: Yes 的块的声明路径并集（与 ReleaseDeclaredDiffCheck 同一个集合）
                          #   判定（逐 cohort、逐站点留证据，顶层结论是各 cohort 的合取）：
                          #     Failed  —— 任一非 Reachable 的项，在本次 TargetSites 的某个站点上没有落地方案；
                          #                 或提测后新增的站点未被方案覆盖
                          #     Blocked —— memory/站点自研代码清单.md 缺失/过期，或 SyncReach 里仍有 Undetermined
                          #     Passed  —— 上述都不成立，且证据齐
                          #   🔴 一个顶层 Passed **不得**掩盖某个 cohort 或某个站点的 Failed —— 逐站点结论
                          #      必须落在正文的证据里，与 PerSitePushResult 一样逐站点可查
                          #   规则与判定算法的唯一事实源：references/sync-reach.md
...
五条硬规则（#1–#4 出自 DTC §五；#5 是 v0.4.0 新增，出处见 §8.1 #12）：
...
5. 🔴 **`SyncReachCheck: Passed` 之前不得推站。** 推站成功 ≠ 改动到店：sync 对 15 条保护路径保留各店版本，
   推送本身会正常完成、`RemoteVerifyResult` 会正常 `Matched`、`PerSitePushResult` 会全是 `Succeeded`，
   **而各店页面一个都没变**。这条是矩阵里唯一会去核对这件事的门；跳过它，前面所有的绿灯都不构成「上线了」的证据。
   规则见 `sync-reach.md`。

发版本身（`git push` / Shopify theme push / 合并 PR）是**外部动作**，本 skill 只产出清单与判定，执行需用户显式授权。

> **本块共 29 个 key**（v0.3.6 为 28，v0.2.3 为 16），顺序即上表顺序，是封闭集合。
>
...
| `ReadyForDelivery` | `Yes`（仅 QA，**两种 `QAScope` 都可取**，条件见 §2.11 —— QA 侧第 1–4 条；release 侧第 5–9 条另由 `plaud-theme-release-ops` 守，v0.4.0 起第 9 条是 `SyncReachCheck`）\| `No` \| `N/A(ReadOnly)` |
...
| `ReconMode` | `LegacyImpact` \| `IntegrationSurface` \| `InlineLite` \| `N/A(ReadOnly)`。🔴 **v0.4.0**：命中 `sync-reach.md` §2 保护规则（§2.1 例外除外）的改动**不得**取 `InlineLite`（§3） |
| `SyncReach[].Classification` | `Reachable` \| `NotReachable` \| `NewTemplateOnly` \| `LocaleFieldLevel` \| `SiteOverridden` \| `Undetermined`。**v0.4.0 新增**（§3）。🔴 `SiteOverridden` **不由 glob 产生**：它来自 `sync-reach.md` §5.1 第 6/7 步（路径与项目侧 `memory/站点自研代码清单.md` 里某个店的自研文件同名）。下游把它与 `NotReachable` **同等对待**——它属于「非 `Reachable`」，必须有覆盖那几个店的落地方案。漏掉这一步，已知碰撞（`sections/us-form-contact-sales-2.liquid` / `assets/affirmShopify.js` 在 LATAM 与 US）会被判成 `Reachable` 一路绿灯放行。🔴 **不得为「同时命中多条规则」自造组合值**——多重命中记进 `MatchedRules[]`，结论只有一个 `EffectiveRule` + 一个 `Classification`。🔴 `Undetermined` 是**必须存在的诚实出口**，不是兜底垃圾桶：出现它则 `ReadyForImplement: No` + `BlockingGaps` 非空。判据 `sync-reach.md` §4.1、§5.1 |
| `SyncReach[].ChangeKind` | `Added` \| `Modified` \| `Deleted`。**v0.4.0 新增**。**刻意没有 `Renamed`**：重命名拆成 `Deleted`(旧路径) + `Added`(新路径) 两条，两条都要出现——只记一条 `Renamed` 会漏掉「旧路径在各店留了个孤儿」那一半（`sync-reach.md` §3） |
| `SyncReachStatus` | `Complete` \| `Incomplete` \| `NotApplicable`。**v0.4.0 新增**（§9.1.2 第七项材料）。`NotApplicable` **仅当** `SyncReach` 全部 `Reachable` 且附零命中证据 |
| `SyncReachCheck` | `Passed` \| `Failed` \| `Blocked`。**v0.4.0 新增**（§9.1.4）。🔴 **刻意没有 `NotApplicable`**：发版必然有 ChangeSet，「一条规则都没命中」的正确取值是 `Passed` + 零命中证据；填 `NotApplicable` 会让「查过没命中」与「没查」变成同一个读数 |
| `RiskTier` | `Low` \| `Medium` \| `High` |
