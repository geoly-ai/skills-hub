...
④ 发布门         ReleaseSourceTreeOid == VerifiedThemeTreeOid ← 本 skill 守（handoff-schema §2.11 第二层）
                ...
                + 执行 push 命令紧前再复算一次
⑤ 到店门         SyncReachCheck: Passed                    ← 本 skill 守（v0.4.0 新增，Step 2.5）
```

> 🔴 **门 ⑤ 与前四道门验的不是同一件事。** ①–④ 验的都是「**基线这棵树**对不对、推的是不是它」。
> 门 ⑤ 验的是「**这棵树到不到店**」—— sync 对 15 条保护规则圈住的文件保留各店版本，
> 推送会正常完成、`RemoteVerifyResult` 会 `Matched`、`PerSitePushResult` 会全 `Succeeded`，
> **而各店页面一个都没变**。没有门 ⑤，前四道门全绿也不构成「上线了」的证据。
...
Step 2.5  到店复核（SyncReachCheck，v0.4.0 新增 —— 必须在 Step 2 之后，它要用二次确认后的 TargetSites）
Step 3    汇总 PR
...
### Step 2.5 — `SyncReachCheck`：推站前重判一次到店（v0.4.0 新增）

> 🔴 **推站成功 ≠ 改动到店。**
> sync 对 15 条保护规则圈住的文件**保留各店版本**。推送本身会正常完成、`RemoteVerifyResult` 会正常 `Matched`、
> `PerSitePushResult` 会全是 `Succeeded` —— **而各店页面一个都没变**。
> 本 skill 是矩阵里**唯一**会去核对这件事的最后一道门；跳过它，前面所有的绿灯都不构成「上线了」的证据。
> 规则、匹配语法、例外与分类算法的唯一事实源是 `plaud-theme-shared/references/sync-reach.md`。

**不是抄 `plaud-theme-qa-intake` 的 `SyncReachStatus`。** 那一份判的是**提测那一刻**材料齐不齐；
本步判的是**本次实际 `TargetSites`** 与**当前**的 `memory/站点自研代码清单.md` 下，逐店结论还成不成立。
两者之间隔着 Step 2 的二次确认 —— 站点清单**本来就可能变**，那正是二次确认存在的理由。

**输入**：`IncludedInThisPush: Yes` 的块的**声明路径并集**（与 `ReleaseDeclaredDiffCheck` 同一个集合）。

🔴 **先核覆盖等式**（`sync-reach.md` §5.3.1）：**`IncludedInThisPush: Yes` 的块**各自 `SyncReach` 的路径并集**必须恰好等于**这个输入集合。
（🔴 两侧同源只取 `Yes` 块 —— `No` 块两侧都不参与，否则会与 §2.14 的剔除语义打架。）

🔴 **还要核两条，否则等式可以在真空里、或被路径去重掩盖着成立**：

1. **`IncludedInThisPush: Yes` 至少一个。** 一个都没有时并集是空集，「等式成立」「每条都有方案」
   全部在真空里为真 → `SyncReachCheck` 会得到一个毫无意义的 `Passed`。空的 `Yes` 集合一律停机。
2. 🔴 **`Yes` 块与 `No` 块的声明路径必须两两不相交。** 相交时那条路径会出现在 `Yes` 并集里，
   于是 `ReleaseDeclaredDiffCheck` 看到它「已被声明」而不判 `DECLARED_DIFF_ORPHAN` ——
   **`No` 块那一半未验收的改动就这样被去重掩盖着推上线了**。相交即停机，
   要求集成者把 `No` 块从树里真正撤掉、重新取证、重跑集成 QA（§2.14），
   **不得**因为「反正那条路径 `Yes` 块也声明了」就放行。
不等 → `SyncReachCheck: Blocked`，**不是** `Passed`。
少一条 = 那条路径从没判过，而下游会把「没出现」读成「`Reachable`」→ 一路绿灯；多一条 = 拿的是上一轮的结果。
**这一步要在逐站点判定之前做** —— 集合本身都不对，逐站点判得再细也只是在判一个残缺的集合。

**逐 cohort、逐站点**判定，顶层结论是各 cohort 的合取：

| 取值 | 条件 |
|---|---|
| `Failed` | 任一**需要方案**的项（判据见 `sync-reach.md` **§4.2**：`Classification` + `ChangeKind` 一起看，**不是**「非 `Reachable` 就要」），在本次 `TargetSites` 的某个站点上没有落地方案；或**任一 `OverriddenAtSites` 非空的项**，在 **`OverriddenAtSites ∩ TargetSites`** 的某个店上没有方案（§4.2 末行铁律，**与 `Classification` 无关**）；或二次确认后**新增**的站点未被提测时那份方案覆盖 |
| `Blocked` | `memory/站点自研代码清单.md` 缺失或 `AsOf` 过旧；或上游 `SyncReach` 里仍有 `Undetermined` |
| `Passed` | 上述都不成立，且证据齐（零命中时也是 `Passed` + 零命中证据） |

🔴 **本字段刻意没有 `NotApplicable`。** 发版必然有 ChangeSet，「一条保护规则都没命中」的正确取值是
`Passed` + 零命中证据。填 `NotApplicable` 会让「查过、没命中」和「根本没查」变成同一个读数。

🔴 **一个顶层 `Passed` 不得掩盖某个 cohort 或某个站点的 `Failed`。**
逐站点结论必须落在正文的证据里，与 `PerSitePushResult` 一样逐站点可查。
只写一个顶层结论 = 站点集合越大越容易蒙混。

**最容易在这一步才暴露的两种情况**（前面每一道门都看不见）：

1. 二次确认把 `TargetSites` 从 3 个店扩到 12 个店 —— 提测时的落地方案只写了那 3 个店。→ `Failed`
2. 本次新建了 `sections/us-form-contact-sales-2.liquid` 或 `assets/affirmShopify.js` ——
   **LATAM 与 US 各有一个同名自研文件**，引擎结构性避让，那两个店永远拿不到基线版。
   → 逐站点结论必须把这两个店单独标出来，不能被 `Passed` 盖掉。
...
>
> `git push`、`shopify theme push`、合并 PR 都是**不可逆的外部动作**，必须等用户看过清单后显式授权才执行。五道门全绿也不构成自动执行的许可。
...
| `SyncReachCheck` 不是 `Passed`（`Failed` / `Blocked` 都算） | 停，不推。🔴 `Blocked` 是"该验但验不了"，**不得折算为 pass** —— 与 `ReleaseDeclaredDiffCheck` 同一条纪律 |
| `QAIntake` 里没有 `SyncReachStatus`，或某块**存在** §3 工件却缺 `SyncReach`（v0.3.6 及以前的旧工件） | 停。`SyncReachCheck: Blocked`，要求按当前 `ContractVersion` 重出上游工件。**不得**因为"旧工件没这个字段"就当它通过。🔴 **`ReconMode: InlineLite` 的块本来就没有 §3 工件**，不适用本条：它的到店证据是实现工件里 InlineLite 第 9 项的逐路径核查结果（零命中），照该证据判 `Passed` |
| 二次确认后 `TargetSites` 新增了站点，而提测时的落地方案没覆盖它们 | 停，`SyncReachCheck: Failed`，`BlockingGaps` 列出漏掉的站点。**这正是本步要抓的东西**，不要因为"提测时是 Complete"就放行 |
| 站点清单只确认过一次 | 停，补第二次确认 |
| 站点清单无出处 | 停，要出处 |
| 有站点在 Target 与 Excluded 里都没出现 | 停，确认遗漏 |
| 没有 PR | 停，要 agency 提供 |
| 用户要求跳过 QA 紧急上线 | **不出发版清单、不给"可以推"的结论**（五道门未过）。如实列出缺了哪些验证与风险，给出最短合规路径（`references/release-checklist.md` §4）。`ReleaseScope[].QAConclusion` 照实写"缺失"，**不得**伪造。推送是用户自己的动作，本 skill 不代为编排、不为它背书 |
...
SyncReachCheck:           # v0.4.0：Passed | Failed | Blocked（**没有 NotApplicable**：零命中的正确取值
                          #   是 Passed + 零命中证据）。推站前**重判**一次，不抄 QAIntake 的 SyncReachStatus ——
                          #   那一份判的是提测那一刻的材料，本字段判的是**二次确认后的 TargetSites** 与
                          #   **当前**的 memory/站点自研代码清单.md 下逐店结论还成不成立。
                          #   期望输入 = IncludedInThisPush: Yes 的块的声明路径并集（与 ReleaseDeclaredDiffCheck 同一集合）。
                          #   🔴 逐 cohort、逐站点留证据；顶层 Passed 不得掩盖某个站点的 Failed。
                          #   规则唯一事实源：plaud-theme-shared/references/sync-reach.md（见 Step 2.5）
...
> **本块共 29 个 key**（v0.3.6 为 28、v0.2.3 为 16），顺序即上表顺序，是**封闭集合**——多一个 key 或少一个 key 都是契约违规。
