...
**何时读我**：执行 Step 2–5 时。五道门的判定在 SKILL.md 正文（v0.4.0 起门 ⑤ 是 SyncReachCheck）。
...
| 8b | **到店复核**（v0.4.0） | `SyncReachCheck: Passed`，且逐 cohort 逐站点证据齐（见 §2.1）。🔴 前面每一项验的都是「基线这棵树对不对」，只有这一项验「这棵树到不到店」 |
...
🔴 **两次清单不一致时还要重跑 §2.1 的到店复核。** 第二次确认**新增**的站点，在提测时那份逐店落地方案里
没有覆盖 —— 这正是 `SyncReachCheck` 要抓的第一种情况。只点出差异而不重判，等于把新站点当成"反正规则一样"放过去。

---

## 2.1 到店复核（`SyncReachCheck`，v0.4.0）

> 规则、匹配语法、例外与分类算法的唯一事实源是 `plaud-theme-shared/references/sync-reach.md`。本节不复制规则表。

**为什么在这里而不是在 QA**：QA 验的是「基线这棵树对不对」。sync 对 15 条保护规则圈住的文件**保留各店版本**，
所以推站会正常完成、`RemoteVerifyResult` 会 `Matched`、`PerSitePushResult` 会全 `Succeeded`，**而各店页面一个都没变**。
门 ①–④ 一道都不会响。本步是矩阵里唯一会去核对这件事的地方。

**输入**：`IncludedInThisPush: Yes` 的块的**声明路径并集**（与 `ReleaseDeclaredDiffCheck` 同一集合）。

### ✅ 合格

```yaml
SyncReachCheck: Passed
# 证据（正文，逐 cohort 逐站点）：
# cohort A（plaud-us / plaud-de / plaud-jp）
#   sections/sa-product-hero.liquid        Reachable      MatchedRules: []
#   assets/sa-product-hero.css             Reachable      MatchedRules: []
#   locales/en.default.json
#     └ sections.sa_product_hero.title     Added   → LocaleFieldLevel + Added = 到得了店，§4.2 判定「不要方案」
#     └ sections.sa_product_hero.cta       Modified→ LocaleFieldLevel，改已本地化键值**到不了店**
#        落地方案：plaud-us 运营 @a 于 2026-09-05 后台改；plaud-de 同；plaud-jp 本次不改该文案（运营 2026-09-03 确认）
#        出处：LIN-1234 / 飞书群 2026-09-03
#   已比对 sync-reach.md §2 全部 15 条；例外表 §2.1 无命中
```

### ❌ 不合格

```yaml
SyncReachCheck: Passed          # 但正文只有一句"本次改动不涉及受保护路径"
SyncReachCheck: NotApplicable   # 本字段没有这个取值
SyncReachCheck: Passed          # 顶层 Passed，而 cohort B 里 plaud-tw 其实没有方案
```

三条都是同一个毛病：**把「查过、没命中」和「根本没查」写成了同一个读数。**

### 判定

| 取值 | 条件 |
|---|---|
| `Failed` | 任一**到不了店**的项（判据见 `sync-reach.md` **§4.2**：`Classification` + `ChangeKind` 一起看，含 **`SiteOverridden`**），在本次 `TargetSites` 的某个站点上没有落地方案；或二次确认后**新增**的站点未被方案覆盖 |
| `Blocked` | `memory/站点自研代码清单.md` 缺失或 `AsOf` 过旧；上游 `SyncReach` 里仍有 `Undetermined`；上游工件是 v0.3.6 及以前的旧版本（根本没有这些字段） |
| `Passed` | 上述都不成立，且逐 cohort 逐站点证据齐（**零命中时也是 `Passed`** + 零命中证据） |

🔴 **`Blocked` 不得折算为 pass** —— 与 `ReleaseDeclaredDiffCheck` 同一条纪律。
🔴 **顶层 `Passed` 不得掩盖某个 cohort 或某个站点的 `Failed`**：逐站点结论必须落在正文证据里，
与 `PerSitePushResult` 一样逐站点可查。站点集合越大，只写顶层结论越容易蒙混过去。

### 站点自研代码碰撞 = `SiteOverridden`，不是靠人记得看一眼

🔴 **这类路径一条 glob 都不命中。** `sections/us-form-contact-sales-2.liquid` 与 `assets/affirmShopify.js`
不在 §2 的 15 条保护规则里，只跑 glob 会给它们 `Reachable` —— 而它们**同时存在于 LATAM 与 US**
的自研代码里（2026-09-03 快照），引擎结构性避让，那两个店永远拿不到基线版，基线侧一切检查正常。

所以它由 `sync-reach.md` §5.1 的**第 6/7 步**产出 `Classification: SiteOverridden`，
下游与 `NotReachable` **同等对待**：必须有覆盖那几个店的落地方案，否则 `SyncReachCheck: Failed`。

本步要核的是：
- 上游 `SyncReach` **确实跑过第 6/7 步**（`SiteOverridden` 项的 `OverriddenAtSites` 逐店列出，或明确记录了零碰撞）；
- 拿不到 `memory/站点自研代码清单.md`、或它的 `AsOf` 过旧 → `Blocked`，**不得**默认「没查到同名 = 没有碰撞」；
- 逐站点结论里把被覆盖的店单独标出来，不能被一个顶层 `Passed` 盖掉。
...
| 环节 | 能压到多短 |
|---|---|
| Assess | 满足 `InlineLite` 九条件时可内联——**Assess 是唯一可豁免的阶段** |
