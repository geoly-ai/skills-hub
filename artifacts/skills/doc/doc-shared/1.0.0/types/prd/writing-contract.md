...
来源：~/.claude/skills/lark-doc/references/genres/prd.md（体裁 workplace.prd）
读取日期：2026-09-15。本文件是该来源文件当日内容的快照整理，lark-cli 升级后若原文变化，本文件不随之自动更新（见 decisions.md ⑨）。
...
用于方向与投入原则已定后，让产品、设计、研发和测试就用户问题、范围、可观察行为与完成标准形成共识。是否立项 / 选择方向 / 批资源走 proposal.md（本平台对应 MRD 或立项材料）；架构、接口和实现取舍走 technical-doc.md（本平台对应 tech-spec）；已批准重复操作走 sop-tutorial.md。"需求 / 功能"单词本身不触发。
...
- REQ 编号体系、章节 id 与 must_answer 由 skeleton.md 定义，原文未规定具体编号格式。
- 「优先级分布合理（不是全 P0）」「至少一张用户流程图」为本包质检新增，原文未强制。
- DocMark 语法（表格编号、图编号、锚点交叉引用、summary 块）与写作体裁无关，遵循 design.md §3.3，不属于本快照范围。
