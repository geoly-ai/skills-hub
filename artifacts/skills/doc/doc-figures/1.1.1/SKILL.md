...
lint 库函数（`sys.path.insert(0, "<doc-figures>/scripts"); import lint`）：
...
```sh
python3 <doc-figures>/tests/run_tests.py [--no-wb] [--keep]
```
...
预拉取：`python3 <doc-figures>/scripts/mermaid.py --ensure-js`（打印路径）；`--cache-path` 只打印缓存路径。
