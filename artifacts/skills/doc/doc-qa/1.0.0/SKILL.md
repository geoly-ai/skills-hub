...
1. 自动检查：`python3 ~/.claude/skills/doc-qa/scripts/qa.py <运行目录>`（类型取 doc.json type；售前等无 doc.json 的目录加 `--type`）。
...
```sh
python3 ~/.claude/skills/doc-qa/tests/run_tests.py          # 全量（含 golden 与售前等价，约 1 分钟）
python3 ~/.claude/skills/doc-qa/tests/run_tests.py --fast   # 跳过 golden 与售前等价
```
