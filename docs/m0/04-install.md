...
⚠️ ① 说的是「本次命令的**全部** target」。只把当前这一个传进去等于取消了这一条 ——
两个互相嵌套、但内层**尚无** `.geoly/` 状态的 target（首次安装时正是这个形状）
判据 ② 也认不出来，于是两个都被放行。

#### 3.5.1 扫描预算与「扫不完」

判据 ② 要向下遍历，遍历必须有界。**上限是实现的资源闸，不是规则**：

| 预算 | 默认 | 硬顶 | 旋钮 |
|---|---|---|---|
| 深度 | 64 | 1024 | `--scan-max-depth <N>` |
| 目录数 | 100000 | 5000000 | `--scan-max-dirs <N>` |
| 目录项 | 1000000 | 50000000 | —（`maxDirs` 拦不住「单个目录下几百万条目」） |

🔴 **成本是 O(目录数)，与深度无关**，所以目录数/目录项才是预算，深度只是防病态路径的
sanity guard。把深度当预算收费会误伤正常现场：一个 skill vendored 了一个仓库，
深度就到 12 —— 早期的 `深度上限 8` 因此让**每一次** install 都退出。

🔴 **扫不完 ⇒ 拒绝**（`target.nested-scan-incomplete`）。「扫不完」与「扫完了没有」
是两件事，把前者说成后者就是在假装证明了一个否定命题。**没有关闭开关**，
只有抬高预算的旋钮 —— 抬高之后仍然是一次真扫描、仍然 fail-closed。

🔴 报错必须点名**撞的是哪一个**上限、它的**实际值**、以及**能做什么**。
三种停因（深度到顶 / 目录数或目录项用尽 / 子目录读不进去）的下一步互不相同：
前两种抬预算有用，第三种是权限问题，抬多少都没用。
一条把三者混成一句的报错指导不了任何行动。同一份记账要同时出现在
人读的文案与 `--json` 的 `violation.detail` 里，不能各算各的。
