...
> ⚠️ **下图第 3 行的 `login（device flow）` 已不适用**（2026-09-05 拍板，见
> [`10-open-questions.md`](10-open-questions.md) Q4 与本文 §9 的偏离说明）：
> **不注册应用，`publish` 直接用你已有的 GitHub token，不做 `login` / `logout`。**
> 那一步现在是「**解析已有 token → 披露它的权限面 → 要你确认**」。
> 其余各步不变。
>
> 📌 图**原样保留**：改掉就看不出偏离在哪，也看不出当初定的是什么。
...
| `shell` / `credentials` / `writes-repo` | Tier 2 · **两名**维护者 approve（见下面的豁免） |

🔴 **审批豁免有两条路，都只跳过「要几个人点头」，不跳过任何内容门。**

| 路径 | 触发 | 拍板 |
|---|---|---|
| `author` | 豁免名单里的人**自己投的稿** | 2026-09-01 |
| `approver` | 豁免名单里的人**在这张 PR 上投了当前有效的 approve** | 2026-09-10 |

⚠️ **第二条是一次实质扩大，不是第一条的延伸。** 第一条覆盖「我发我自己的东西」，
那本来就在名单持有人控制之下；第二条覆盖**替别人放行** —— 一张外部 Tier 2 投稿
（声明了 shell / 凭据 / 写仓库能力，装到每台机器上都能执行任意命令）原本要两双眼睛，
现在名单上的人一票即可。**残余风险**：名单上的账号被接管 = 对方可以让任意外部投稿
过审批门。这是在明知代价的前提下选的形态，不是疏漏。

🔴 判据是**当前有效**的 approve（按 PR head sha 过滤、按维护者名单过滤）。
用历史票的话，一张被 push 冲掉的旧 approve 还能继续放行 —— 那正是 §7 那条
「第二票被 dismiss 之后两名就这么没了」的同一个形态换了张面孔。

🔴 放行时**必须在日志里说清是哪条路**（`reason=author` / `reason=approver`）：
