...
**清单在 [`registry/catalog.md`](../../registry/catalog.md)** —— 全部可安装制品的
id、tier、支持的 client 与一句话说明，由 promote 从快照生成，跟着每次发布自动更新。
🔴 **这一行是静态指针，清单本身是生成物**：不要把制品列表抄到这份文档里，
抄一份就多一处会与快照分叉的东西。
...
🔴 **默认开**。`install` 成功收尾后静默发一次：24 小时最多一次、超时 3 秒、失败 1 小时后可再试、
**失败不影响安装**。别的命令只写本地。
