...
🔴 **2026-09-01 起这一格比 Codex 当初描述的更敞。** 当天分支保护从经典模式换成
两条 Ruleset，并按用户要求给 `chovizzz` 加了**审批豁免**（`bypass_mode: always`），
所以：

| 谁的 PR | 需要几名 approve |
| --- | --- |
| chovizzz | **0**（可直接 push 到 main，GitHub 只回一行 `Bypassed rule violations`） |
| 其他维护者 / 外部投稿 | 1（+ CODEOWNERS） |

也就是说，**改这些授权根文件目前不需要任何人复核**。`tier-gate.mjs` 的
「两名且排除作者」只作用于投稿载荷，管不到这些文件。

⚠️ 这不是 `pr-classify.mjs` 能修的，也**不该**靠收回豁免来修 —— 豁免是用户
明确要的（不想为每个日常改动等人审）。真正对症的是一条**只针对授权根文件**的门：
命中清单时要求两名不同维护者、排除作者，其余 PR 不受影响。那样日常改动仍然
一键自合，而「改门本身」这一小类才需要第二双眼睛。
